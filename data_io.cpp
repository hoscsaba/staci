...
void data_io::load_system(vector<Csomopont *> &cspok, vector<Agelem *> &agelemek)
{
    ...
    if (debug)
        cout << endl << "cpp_xml_debug=" << Node_settings.getChildNode("cpp_xml_debug").getText()
             << " => adatfajl olvasasa kozbeni debug bekapcsolasa..." << endl;
    ...
    for (int i = 0; i < csp_db; i++)
    {
        ...
        if (is_endnode == "false")
        {
            ...
            ro = string_to_double(Node_nodes.getChildNode("node", i).getChildNode("density").getText(), id, "<pressure>", 0.0);
...
//                cout << cspok.at(j)->Info();
    ...
    if (debug)
    {
        for (int i = 0; i < ag_db; i++)
        {
            ...
            {
                db = Node_edges.getChildNode("edge", i).getChildNode("edge_spec", 0).nChildNode(edge_type.at(j).c_str());
                if (db == 1)
                {
    ...
    for (int i = 0; i < ag_db; i++)
    {
        ...
        */
        ...
        for (int j = 0; j < edge_type_number; j++)
        {
            ...
            if (db == 1)
            {
                XMLNode elem = Node_edges.getChildNode("edge", i).getChildNode("edge_spec").getChildNode(edge_type.at(j).c_str());
                switch (j)
                {
                case 0: //press = Konst_Nyomas
                {
                    ...
                    if (pres > pmax)
                        pmax = pres;
                    darab++;
                    agelemek.push_back(new KonstNyomas(id, aref, node_from, density, pres, mass_flow_rate, travel_time));
                    if (debug)
                        cout << " OK";
                    break;
                }
                ...
                case 1: //pipe = Cso
                {
                    ...
                    agelemek.push_back(new Cso(id, node_from, node_to, density, L, D, erdesseg, cl_k, cl_w, mass_flow_rate));
                    if (debug)
                        cout << " OK";
                    break;
                }
                ...
                case 6: // kor rm.
                {
                    ...
                    agelemek.push_back(new Csatorna(id, node_from, node_to, density, dia * dia * M_PI / 4., L, ze, zv, erdesseg, int_steps, debugl, dia, cl_k, cl_w, is_reversed, mass_flow_rate));
                    break;
                }
                ...
                case 8: // Buko mutargy
                {
                    ...
                    if (debug)
                        cout << endl << "is_opened:" << iso << ", width=" << wi
                             ...
                             << vc;
                    agelemek.push_back(new BukoMutargy(id, node_from, node_to, density, aref, bh, is_opened, wi, oh, dc, vc, mass_flow_rate));
                    if (debug)
                        cout << "  OK";
                    break;
                }
                case 9: // visszacsap� szelep
                {
                    ...
                    agelemek.push_back(new VisszacsapoSzelep(id, node_from, node_to, density, aref, lce, lcv, mass_flow_rate));
                    if (debug)
                        cout << "  OK";
                    break;
                }
        ...
        for (unsigned int j = 0; j < agelemek.size(); j++)
        {
            ...
            if (id == (agelemek.at(j)->Get_nev()))
            {
                ...
                if (agelemek.at(j)->Get_Tipus() == "Csatorna")
                {
                    ...
                    Node_edges.getChildNode("edge", i).getChildNode("node_from").addText((agelemek.at(j)->Get_Cspe_Nev()).c_str());

                    Node_edges.getChildNode("edge", i).getChildNode("node_to").deleteText();
                    Node_edges.getChildNode("edge", i).getChildNode("node_to").addText((agelemek.at(j)->Get_Cspv_Nev()).c_str());
                }
...
void data_io::save_mod_prop(vector<Csomopont *> cspok, vector<Agelem *> agelemek, string eID, string pID)
{
    ...
    cout << endl << "Kimeneti fajl neve:" << xml_fnev << ", modositott adatok mentese...";
...
//--------------------------------------------------------------------------------
double data_io::string_to_double( const string &s , const string &elem_name, const string &tag_name, const double &def_value)
{
