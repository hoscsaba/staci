...
// double Csatorna::f_simple(vector<double> x) {
...
//--------------------------------------------------------------
// vector<double> Csatorna::df_simple(vector<double> x) {
// 	// double pe=x[0]*ro*g;
// 	// double pv=x[1]*ro*g;
// 	double he = x[2];
// 	double hv = x[3];
	...
	while (!last_step) {
		...
		while (hiba > hiba_max) {
			...
			if ((hiba > hiba_max) || (y2 < 0.)) {
				...
				// 	last_step = true;
				// 	strstrm << endl << "!!! A " << nev
				// 	        << " csatornaban y=" << y2 << "m, tomegaram: " << mp << " kg/s" << endl;
				// 	error("ode_megoldo()", strstrm.str());
				// }
			}
			else {
 ...
 * @return �gegyenlet �rt�ke
 */
double Csatorna::f_1_old(double ye, double yv, double mp) {
	//  Matlab k�d
	//  eval_jac=[1 1 1];
 ...
 */
double Csatorna::f_2c_old(double ye, double yv, double mp) {
	ostringstream strstrm;
 ...
 * @return error of branch equation
 */
double Csatorna::f_2ai(double ye, double yv, double mp) {
	ostringstream strstrm;
