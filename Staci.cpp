...
double Staci::m_get_dprop() {
  ...
  if (!elem_megvan) {
    cout << endl
         << endl
         << "HIBA!!! Staci::m_get_dprop(): Nincs ilyen elem: " << element_ID
         << endl
         << endl;
    exit(-1);
  } else {
    if (!prop_megvan) {
      cout << endl
           << endl
           << "HIBA!!! Staci::m_get_dprop(): Elem: " << element_ID
...
void Staci::build_system() {
  ...
  msg1 << endl
       << " Number of nodes: "
       << cspok.size();  // << " (capacity: )" << cspok.capacity();
  msg1 << endl
...
string Staci::iter_info(Vec_DP x, Vec_DP f, int iter, double e_mp, double e_p) {
  ...
  if (debug_level > 3) {
    for (unsigned int i = 0; i < agelemek.size(); i++)
      m_ss << "\n\t" << agelemek[i]->Get_nev() << "(" << agelemek[i]->GetType()
           << "): mp=" << x[i] << ", f=" << f[i];
    for (unsigned int i = 0; i < cspok.size(); i++)
      m_ss << "\n\t" << cspok[i]->Get_nev() << ": p=" << x[agelemek.size() + i]
           << ", f=" << f[agelemek.size() + i];
  }
...
void Staci::Compute_dfdmu() {
  ...
  for (int i = 0; i < agelemek.size(); i++) {
    if ((property_ID == "diameter") &&
      ...
      // cout<<endl<<"Hali! element:"<<agelemek.at(i)->Get_nev()<<",
      // dfdmu="<<dfdmu;
      // cin.get();
...
void Staci::Compute_Sensitivity_Matrix(string parameter, int scale) {
  ...
  if (strcmp(property_ID.c_str(), "consumption") == 0) is_edge_prop = 1;

  if (is_edge_prop == -1) {
    cout << "\n\n ERROR!\n\nCompute_Sensitivity_Matrix(string parameter) -> "
         "unknown parameter: "
         << parameter;
    cout << "\n possible values: diameter|friction_coeff|consumption" << endl
         << endl;
    exit(-1);
  }
